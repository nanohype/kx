...
- Don't add CI that needs a live cluster or cloud credentials. Anything that requires a running kind cluster stays a local `task` target. `.github/workflows/ci.yml` runs five jobs behind a merge gate — lint (yamllint, shellcheck, ruff), Renovate pin coverage, chart provenance, the eks-gitops mirror comparison, and a clusterless `helm template` render gate over every slice followed by mount and schema validation of what it rendered. The questions whose answer changes without this repository changing — has a chart been deprecated upstream, do the pinned images carry a CRITICAL, has the catalog moved past the pin, has the eks-agent-platform sibling — run on a schedule in their own workflows, because asking them on the blocking path reddens pull requests that did not cause them
